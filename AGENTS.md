...
- Observability: throw via `Err.*`; they capture to Sentry and, in a consenting browser, Pulse. `captureException` is called
  directly only in `app/error.tsx` / `app/global-error.tsx`, for non-`AppError` values; no raw user data. `docs/sentry.md`
